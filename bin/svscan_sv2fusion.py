...
def main():
    ...
    of = open(fs,'w')
    header = ['SampleID','FusionGene','Headgene','Tailgene','FusionReadsNumber','TotalReadsNumber','FusionRate(%)','COSMIC','OncoKB','IfReport','Gene1','Chr1','JunctionPosition1','Strand1','Transcript1','Region1','Gene2','Chr2','JunctionPosition2','Strand2','Transcript2','Region2','FusionSequence','SVType','svSize','insSeq','insLen','FusionOriPattern']
    ...
    for line in infile:
        ...
        for i in gene_pair:
            ...
            if hgene == 'NTRK1' or tgene == 'NTRK1':
                # NTRK1基因总深度为0,重新计算
                bnd_left_depth = cal_fusion_ref_depth(val[3],int(val[4]),args.bam,args.outdir) # 分子数
                bnd_right_depth = cal_fusion_ref_depth(val[5],int(val[6]),args.bam,args.outdir)
            ...
            if in_cosmic == 'Y':
                ...
                # 检查断点深度,有一端为0则过滤掉
                bnd_left_depth = check_bnd_depth(val[3],int(val[4]),args.bam,args.outdir)
                bnd_right_depth = check_bnd_depth(val[5],int(val[6]),args.bam,args.outdir)
            ...
            val_line = name + '\t' + fgene + '\t' + hgene + '\t' + tgene + '\t' + str(fnum) + '\t' + str(tnum) + '\t' + str(ffreq) + '\t' + in_cosmic + '\t' + in_onckb + '\t' + if_report + '\t' + g1_val + '\t' + g2_val + '\t' + fseq + '\t' + svtype + '\t' + str(svsize) + '\t' + ins_seq + '\t' + str(ins_len) + '\t' + fp
...
def check_bnd_depth(chrom,pos,bam,outdir):
    # 检查断点及断点两侧各1bp位置的深度
    r = "%s:%s-%s" % (chrom,pos-5,pos+5)
    cmd = "samtools depth -a -r %s %s >%s/bnd.depth.tmp" % (r,bam,outdir)
    os.system(cmd)
...
def cal_fusion_ref_depth(chrom,pos,bam,outdir):
    # used for cal NTRK1's depth
    r = "%s:%s-%s" % (chrom,pos-10,pos+10)
    cmd = "samtools view %s %s | awk '{print $1}' | sort | uniq -c | wc -l >%s/samtools_depth.tmp" % (bam,r,outdir)
    os.system(cmd)
